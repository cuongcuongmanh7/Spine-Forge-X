  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (firebaseConfigured() && !captureRegistry.has(key)) void uploadThumb(key, url).catch(() => undefined);
      } catch {
        fail();
      }
    })();
  ...
  useEffect(() => {
    ...
    const warmOne = async (entry: LibraryEntry) => {
      ...
      if (!captureIsStale(key)) {
        try {
          const cached = await invoke<string | null>('thumb_cache_get', { key });
          if (cancelled || cached) return; // already local & current → nothing to pull
        } catch {
          /* treat as miss */
        }
      }
