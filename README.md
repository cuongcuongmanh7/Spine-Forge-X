...
- **Output policies** — choose where each file lands: a single output root keyed by source folder name, a Linked Project (Unity) tree, or an `export` subfolder created next to each `.spine` file (no output root needed).
- **Quick export from the Library** — one click in any row's `⋯` menu (file → that file, folder → the whole group) exports immediately using the preset + output currently selected in the Workspace tab, without creating a session.
...
- **Cơ chế output** — chọn nơi xuất từng file: một output root tổng theo tên thư mục nguồn, cây Linked Project (Unity), hoặc thư mục `export` tự tạo ngay cạnh từng file `.spine` (không cần output root).
- **Quick export từ Thư viện** — bấm một lần ở menu `⋯` của mỗi dòng (file → đúng file đó, thư mục → cả nhóm) để export ngay bằng preset + output đang chọn ở tab Workspace, khỏi tạo session.
...
- **Asset Library (Spine Hub)** — kiểm kê mọi `.spine` dưới thư mục thư viện (version, dung lượng, animation/skin) ở dạng bảng hoặc **lưới thẻ kèm thumbnail skeleton thật**, tìm theo `anim:`/`skin:`, tab gom version lẫn lộn, "dùng bởi N project", gắn tag + người phụ trách kèm lọc theo tag/người, dữ liệu Google Drive (người sửa cuối / lịch sử / mở bản cũ), và **xem trước skeleton trực tiếp** bằng Spine web player (3.8 + 4.x). Dữ liệu đồng bộ vào một thư mục Google Drive cố định (tự dò) — **workspace riêng từng người** (project/session, theo tài khoản Google) tách khỏi **thư viện dùng chung của nhóm** (danh sách + tag + thumbnail).
