...
## Unreleased
- **Đồng bộ dữ liệu app qua Google Drive (Sync — Tier A)**: animator làm ở công ty rồi về nhà làm tiếp mà không phải set-up lại — toàn bộ **project/session/cấu hình** được mirror vào một file `spineforge-profile.json` trong thư mục Google Drive dùng chung; đường dẫn source spine lưu tương đối theo token `${SPINE_ROOT}` và **tự ghép lại đúng ổ đĩa** của từng máy (G:\ vs H:\). Một **gốc Drive chung** (vd `G:\Shared drives`) vừa là nơi đặt file, vừa là gốc rebase — nên nhiều project ở các shared drive khác nhau (FD/DH) đều portable. Toggle **mặc định bật** và **tự dò** `\Shared drives\` lần đầu (cảnh báo nếu không dò được). Reconcile newer-wins lúc khởi động + debounce ghi khi có thay đổi, có guard để máy mới (rỗng) không đè dữ liệu lên profile máy cũ. **Status dot** trên titlebar (xám/vàng/xanh/đỏ) + mục Settings ▸ Sync. Backend thêm `system::read_text_file` / `detect_drive_root`. Machine-local (Spine.exe path, vị trí gốc Drive) không đồng bộ. Chi tiết: [docs/sync.md](docs/sync.md). Kèm test rebase + i18n vi/en.
- **Base preset mặc định = binary + pack**: preset mới tạo từ editor giờ mặc định skeleton **binary (.skel) + packed atlas** thay vì JSON, khớp với built-in export.
- **Sửa Library: header bảng bị dòng cuộn đè**: `.library-table` chuyển `border-collapse: collapse → separate` (collapsed-border khiến Chromium bỏ qua `z-index` của sticky header). Group-row (band folder/ID) cũng dính tầng dưới header khi cuộn list dài.
