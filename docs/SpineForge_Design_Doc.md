...
## XI. ĐỌC LAST-EXPORT SETTINGS TỪ .SPINE (v0.2.14, mở rộng v0.2.15, v0.2.18)

Mode export `lastExportSettings` lấy settings export riêng của từng project thẳng từ file `.spine`, bỏ nhu cầu save `.export.json` thủ công cho mỗi file.

> **Đính chính (2026-06):** tài liệu nội bộ trước đây ghi "Spine CLI có flag `--last-export-settings` từ editor 4.3+" — **sai**. Xác minh trên [CLI docs chính thức](http://en.esotericsoftware.com/spine-command-line-interface) + forum: Spine CLI **không** có cờ nào dùng settings lưu trong project, ở mọi version. Cờ `-e/--export` chỉ nhận đường dẫn `.export.json` hoặc tên built-in (`binary`/`json` với default settings). Bản 4.1.10-beta chỉ thêm việc *dump* output path qua `spine -i project.spine`, không phải export theo settings nội bộ. ⇒ Cách **duy nhất** áp settings per-project (cho cả 3.8 lẫn 4.x) là tự parse binary `.spine` → merge → sinh temp `.export.json`. Parser hiện chỉ validate trên **3.8.x**; hỗ trợ format 4.3.x là backlog (cần file 4.3 thật + bản export editor để reverse-engineer và validate byte-identical).

> **⚠️ Giới hạn độ tin cậy (v0.2.15, validate trên 3001_Lucius):** giá trị trong `.spine` chỉ chính xác **ngay sau khi export từ Export window trong Spine editor** — thao tác đó mới ghi pack settings ngược vào project (đã chứng minh: sửa max+scale trong editor rồi export → block trong file đổi theo). Export qua **shell script / SpineForge preset / CLI `-e`** **không** ghi ngược → giá trị giữ nguyên trạng thái cũ (stale). Studio export bằng preset chung thì giá trị trong file vô nghĩa → dùng "Dùng preset cho mọi file" với preset đúng. Mode này chỉ hợp với người set-up-và-export trực tiếp trong editor từng file.

* **Format** (reverse-engineered, validated trên project thật): `.spine` 3.8.x là **một raw deflate stream** (không zlib header → Rust `flate2::DeflateDecoder`). Payload sau inflate là binary serialization riêng: chuỗi ASCII có byte cuối OR `0x80` (`"binar"` + `0xF9` = "binary"). Số nguyên là varint base-128 little-endian **zigzag-encoded** (libGDX `writeInt` với `optimizePositive=false`: n ≥ 0 lưu thành 2n) — phát hiện v0.2.18; float (f32 BE) và bool (1 byte) không bị. Khối settings nằm gần cuối payload.
* **Trích pack min/max** (tier A — tin cậy tuyệt đối): cặp field-id+varint `07 08 09 0A` = minWidth/minHeight/maxWidth/maxHeight, **zigzag-decode** (v0.2.18 sửa bug đọc gấp đôi — trước đó Chest bị báo 256/4096 thay vì 128/2048). Heuristic: 4 giá trị ∈ [16, 16384] (KHÔNG bắt power-of-two — project thật dùng size tùy ý như 700), min ≤ max, theo sau là field `0B <bool>` (pot) làm neo; lấy match **cuối cùng** trong buffer (match sớm là dữ liệu skeleton trùng cờ). Các field chuỗi (extension, packSource/packTarget, outputFormat, atlasExtension, class binary/json) + `cleanUp` cũng tin cậy.
* **Trích `scale`** (field 0x13, v0.2.15, tier A — validated): `13 <count> [02 <f32 BE>]...`, chỉ decode single-scale. Spine lưu scale ở full-res; bỏ qua → export sai độ phân giải (3001 scale 0.5: có scale ra page ~310 đúng editor, bỏ scale ra ~640). Merge override `packAtlas.scale`; multi-scale giữ preset.
* **Trích padding & các field zigzag/bool (v0.2.18, tier A — validated):** `paddingX/Y` (field 0x16/0x17, zigzag), `edgePadding`/`duplicatePadding` (0x18/0x19, bool), `alphaThreshold` (0x06, zigzag), `multipleOfFour` (0x0C, bool), `premultiplyAlpha`/`bleed` (0x11/0x12, bool). Các demote cũ của padding/alphaThreshold/multipleOfFour là do hiểu nhầm zigzag + min/max gấp đôi làm lệch layout, không phải stale.
* **Trích `packing` (field 0x28, v0.2.18, tier A — validated):** encode `28 01 <enum>` với enum 2 = rectangles, 3 = polygons. Editor luôn ghi field này (không omit ở default). Phát hiện kèm: 0003_Althea vốn là Polygons → đó là lý do thật khiến layout nó không reproduce dưới rectangles trong calibration cũ, không phải packing nondeterminism.
* **Merge & fallback:** `merge_decoded_settings` ghi đè per-field lên base preset (pack-atlas key bỏ qua khi preset tắt packing); parse fail toàn phần → export bằng base preset, log lý do. **Cảnh báo divergence (v0.2.15):** pack max parse được ≠ base preset → `spine-log [WARN]` (dấu hiệu file stale). Verify: Chest.spine atlas+PNG **byte-identical**; 3001_Lucius merge thật tái tạo **atlas + toàn bộ PNG byte-identical** với export editor cho cả Rectangles lẫn Polygons (v0.2.18); skel.bytes lệch vùng hash là nondeterminism CLI re-export (độc lập settings — 2 bản editor rect/poly có skel giống hệt nhau).
* **Phát hiện file 4.x (v0.2.20):** payload (cả 3.8 lẫn 4.x) đóng dấu version editor dạng hibit-string ngay đầu (`3.8.99`, `4.3.17`). File save bởi editor 4.x có layout khác (không còn block `07 08 09 0A` + neo `0B`) → scan fail; `detect_editor_version` đọc dấu version để lỗi ghi rõ "save bởi Spine {v} — chỉ hỗ trợ 3.8.x", và note fallback là `[WARN]` trong log (file đó export bằng preset nền). Validate trên `3001_Lucius_4_3.spine` thật. Lưu ý: **target version** 4.3 (`--update 4.3.xx`) với file nguồn 3.8 vẫn hoạt động đầy đủ — chỉ *đọc settings từ file save bởi 4.x* là chưa support (backlog).
* **Code:** parser thuần `src-tauri/src/spine_project.rs`, test ở `src-tauri/src/spine_project_tests.rs` (unit + proptest + regression fixture ghim bytes thí nghiệm + fixture test `#[ignore]` qua env `SPINE_FIXTURE`; dev tool `calibration_merge_dump`); tích hợp ở `resolve_export_plan` → `create_last_export_settings`; command preview `read_spine_export_settings`. Settings JSON sinh ra là UTF-8 **không BOM** (Spine từ chối BOM).
