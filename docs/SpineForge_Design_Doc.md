...
## XI. ĐỌC LAST-EXPORT SETTINGS TỪ .SPINE (v0.2.14, mở rộng v0.2.15)
...
* **Calibration loại field không đáng tin:** `alphaThreshold`, `multipleOfFour` (lệch lần export thật) và **`padding`** (file ghi 16 nhưng export thật dùng 8 — decoded value không reproduce) → decode để xác định layout nhưng **không emit**, luôn lấy từ base preset.
* **Merge & fallback:** `merge_decoded_settings` ghi đè per-field lên base preset (pack-atlas key bỏ qua khi preset tắt packing); parse fail toàn phần → export bằng base preset, log lý do. **Cảnh báo divergence (v0.2.15):** pack max parse được ≠ base preset → `spine-log [WARN]` (dấu hiệu file stale). Verify: Chest.spine atlas+PNG **byte-identical**; 3001_Lucius (scale 0.5) merge thật tái tạo đúng page editor (80/80 region).
* **Code:** parser thuần `src-tauri/src/spine_project.rs` (unit test + proptest + fixture test `#[ignore]` qua env `SPINE_FIXTURE`; dev tool `calibration_merge_dump`); tích hợp ở `resolve_export_plan` → `create_last_export_settings`; command preview `read_spine_export_settings`. Settings JSON sinh ra là UTF-8 **không BOM** (Spine từ chối BOM).
