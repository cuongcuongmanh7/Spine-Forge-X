...
**Còn lại (pha sau):** xoá code đọc file sidecar cũ (chờ cả team lên ≥ v0.4.41 để seed migration không cần nữa); cân nhắc thay `window.location.reload()`/pull-once-on-mount bằng `onSnapshot` real-time cho tags/notes/drive-meta. (Xem mục "Dự kiến" trong [ROADMAP.md](ROADMAP.md).)
