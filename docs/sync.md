...
**Thumbnail (Cloud Storage, 3 tầng).** Mở thẻ Library: (1) **L1** cache cục bộ mỗi máy (Rust `thumb_cache_*`, nhanh, chạy offline, không cần Drive) → trúng thì dùng luôn; (2) **L2** Cloud Storage — dùng thẳng download URL làm `<img>` src (không cần mount Drive, dùng được trên web/mobile sau này); (3) miss cả hai → render WebGL rồi lưu L1 + upload L2 để máy khác khỏi render lại. **Backfill L1→L2 (v0.4.45):** khi trúng L1 nhưng key chưa có trên L2 (thumb chụp trước khi có sync, hoặc render/chụp lúc offline/chưa đăng nhập) → tự đẩy lên L2 (nền, best-effort, mỗi key một lần/máy — ghi nhớ `localStorage` `spineforge.thumbL2Present`), nên các bộ thumb legacy trên nhiều máy chung account **tự gộp thành hợp** thay vì kẹt cục bộ. Key băm theo nội dung → union merge an toàn. Thumbnail là cache nên rule cho **mọi member** ghi (`storage.rules`), `delete:false`.
