...
| Tag/owner + notes + drive-meta cache | Firestore `envs/{dev\|prod}/library/tags\|notes\|drive_<libId>` (v0.4.41 — mọi member ghi được, không leader-only) |
...
**Thumbnail (Cloud Storage, 3 tầng).** Mở thẻ Library: (1) **L1** cache cục bộ mỗi máy (Rust `thumb_cache_*`, nhanh, chạy offline, không cần Drive) → trúng thì dùng luôn; (2) **L2** Cloud Storage — dùng thẳng download URL làm `<img>` src (không cần mount Drive, dùng được trên web/mobile sau này); (3) miss cả hai → render WebGL rồi lưu L1 + upload L2 để máy khác khỏi render lại. **Backfill L1→L2 (v0.4.47):** khi trúng L1 nhưng key chưa có trên L2 (thumb auto-render có trước khi có sync, hoặc render lúc offline/chưa đăng nhập) → tự đẩy lên L2 (nền, best-effort, mỗi key một lần/máy — ghi nhớ `localStorage` `spineforge.thumbL2Present`), nên các bộ thumb legacy trên nhiều máy chung account **tự gộp thành hợp** thay vì kẹt cục bộ. Key băm theo nội dung → union merge an toàn. Chỉ chạy cho key **không** nằm trong capture-registry (`!captureRegistry.has(key)`) để không đè ảnh chụp tay do luồng capture ở v0.4.46 quản. Thumbnail là cache nên rule cho **mọi member** ghi (`storage.rules`), `delete:false`.
...
**3 sidecar còn lại — ĐÃ CHUYỂN (v0.4.41).** `spineforge-library-meta.json` (tags + owner), `spineforge-library-notes.json` (notes), `spineforge-drive-meta.json` (cache owner/last-modified) đã lên Firestore doc `envs/{env}/library/tags|notes|drive_<libId>` (mọi member `org()` ghi được, khác `list`/`clean`/`trash` leader-only). setDoc `merge:true` deep-merge object lồng nên ghi 1 key không đụng key/library khác (bỏ read-before-write race); namespace theo `libraryId` (vá bug 2 library trùng relPath). Migration tự động (seed lần đầu từ file cũ khi doc trống). Code: [libraryMetaSync.ts](../src/libraryMetaSync.ts), [useLibraryTags.ts](../src/useLibraryTags.ts), [useLibraryNotes.ts](../src/useLibraryNotes.ts), [useLibraryDrive.ts](../src/useLibraryDrive.ts); rules [firestore.rules](../firestore.rules); test [libraryMetaSync.test.ts](../src/libraryMetaSync.test.ts). Spec + chia PR: [library-sidecar-firestore.md](library-sidecar-firestore.md).

**Realtime (post-v0.4.41).** tags/notes/drive-meta đã chuyển từ pull-once-on-mount sang `onSnapshot` (giống `library/clean|trash`): thay đổi từ máy khác hiện ngay không cần mở lại tab. tags/notes replace theo doc authoritative (xoá cũng propagate); drive-meta merge (cache cộng dồn). Code đọc file sidecar cũ + seed migration cũng đã dọn (chỉ 1 người dùng nên không cần chờ team). Chi tiết: [library-sidecar-firestore.md](library-sidecar-firestore.md) §7.
