...
**Còn lại (pha sau):** chuyển 3 sidecar còn trên Drive sang Firestore — `spineforge-library-meta.json` (tags + owner), `spineforge-library-notes.json` (notes), `spineforge-drive-meta.json` (cache owner/last-modified) → doc `envs/{env}/library/tags|notes|drive_*` (giống `list`/`clean`/`trash`) để được rules bảo vệ + bớt phụ thuộc mount Drive; thay `window.location.reload()` bằng `onSnapshot` real-time. Spec chi tiết + chia PR: [library-sidecar-firestore.md](library-sidecar-firestore.md). (Việc lớn, tách riêng — xem mục "Dự kiến" trong [ROADMAP.md](ROADMAP.md).)
