...
**Còn lại (pha sau):** cân nhắc thay pull-once-on-mount bằng `onSnapshot` real-time cho tags/notes/drive-meta (giống `library/clean|trash` đã có `subscribe*`). *(Code đọc file sidecar cũ + seed migration đã dọn post-v0.4.41 — chỉ còn 1 người dùng nên không cần chờ.)* (Xem mục "Dự kiến" trong [ROADMAP.md](ROADMAP.md).)
