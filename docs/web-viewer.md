...
- **Đối tượng:** đồng nghiệp non-animator, máy không mount được shared drive theo cách app desktop làm,
  không có Spine 2D CLI để export. Họ chỉ cần *xem* asset trông thế nào.
...
1. **Stack backend:** Node (googleapis, dễ deploy serverless) hay **Rust (axum) tái dùng thẳng code Tier B**?
   (Tradeoff: reuse code Drive vs. hệ sinh thái deploy/Firebase Admin.)
