...
>
> **Cập nhật v0.4.8 — Sync model v2 (thay phần mô tả Tier A bên dưới):** đã chuyển sang **path cố định** `…\Shared drives\Pamvis\spine_app_data` (tự dò, bỏ picker), **workspace tách theo user** (theo email Google, cần đăng nhập) và **library dùng chung** (danh sách thư viện + tag/owner + drive-meta + thumbnail). Tag/owner vẫn là sidecar nhưng nằm trong `library/`. Chi tiết: [sync.md](sync.md) §1–5.
